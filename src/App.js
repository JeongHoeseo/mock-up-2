                ...
                <div className="mb-12">
                  <div className={`border-2 border-dashed rounded-[32px] p-12 text-center cursor-pointer transition-all hover:border-brand-purple group ${theme.uploadZone}`}>
                    ...
                    <label htmlFor="video-upload" className="cursor-pointer">
                      ...
                      <p className="text-lg font-bold mb-2">{videoFile ? videoFile.name : "영상을 업로드하세요."}</p>
                    </label>
                  </div>
                </div>
                      ...
                      <button 
                        ...
                        className={`p-7 rounded-[28px] border-2 transition-all flex items-center gap-5 text-left ${
                          subtitleStyle === option.id 
                            ...
                            : isDark 
                              ? 'bg-[#08090F] border-gray-800/40' // 미선택 시 다크 배경 강제 지정
                              : 'bg-slate-50 border-slate-100'
                        }`}
                      >
                        <div className={`w-7 h-7 rounded-full border-2 flex items-center justify-center transition-colors ${
                          subtitleStyle === option.id ? 'bg-white border-white' : isDark ? 'border-gray-700' : 'border-slate-300'
                        }`}>
                        ...
                        <div>
                          <p className={`font-bold text-lg transition-colors ${
                            subtitleStyle === option.id ? 'text-white' : isDark ? 'text-gray-400' : 'text-slate-600'
                          }`}>
                            {option.name}
                          </p>
                          <p className={`text-[11px] font-black tracking-widest mt-1 uppercase transition-colors ${
                            subtitleStyle === option.id ? 'text-white/70' : isDark ? 'text-gray-600' : 'text-slate-400'
                          }`}>
                            {option.desc}
                          </p>
                        </div>
            ...
            <>
              <section className="flex-1 flex flex-col p-8 gap-8 min-w-0 h-full">
                <div className="flex-1 min-h-0 shadow-2xl rounded-3xl overflow-hidden">
                  <Player url={videoId} isDark={isDark} playing={playing} setPlaying={setPlaying} onDuration={setDuration} onProgress={setPlayed} playerRef={playerRef} />
                </div>
              </section>
