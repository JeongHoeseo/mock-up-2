...
function App() {
  ...
  const updateJobStatusFromJob = (job, jobId) => {
    const transcription = job?.pipeline_result?.transcription;
    const timings = job?.pipeline_result?.timings;
  ...
  const pollJobUntilDone = async (jobId) => {
    ...
    while (true) {
      ...
      updateJobStatusFromJob(job, jobId);
  ...
  const handleStartAI = async () => {
    ...
    if (!cleanBaseUrl || cleanBaseUrl === 'undefined') {
      alert('백엔드 API 주소가 설정되지 않았습니다.');
      return;
    }
    ...
    setJobStatus({
      ...
      status: 'queued',
      step: 'queued',
      message: '작업을 시작하는 중입니다.',
    });

    try {
      const formData = new FormData();

      // 백엔드 FastAPI: file: UploadFile = File(...)
      formData.append('file', videoFile);
      formData.append('domain', subtitleStyle || 'general');
      ...
      if (startRes.data?.job_id) {
        ...
        setJobStatus((prev) => ({
          ...
          status: 'queued',
          step: 'queued',
          message: startRes.data?.message || '영상 처리가 시작되었습니다.',
        }));

        result = await pollJobUntilDone(jobId);
      } else if (startRes.data?.pipeline_result) {
        // 혹시 백엔드가 예전 구조일 때도 임시 호환
      ...
      setJobStatus((prev) => ({
        ...
        llmUsed: transcription?.llm_used ?? prev.llmUsed,
        llmServiceUrl: transcription?.llm_service_url ?? prev.llmServiceUrl,
        llmFallbackUsed:
          transcription?.llm_fallback_used ?? prev.llmFallbackUsed,
        llmFallbackReason:
          transcription?.llm_fallback_reason ?? prev.llmFallbackReason,
        timings: timings || prev.timings,
      }));
  ...
  const styleOptions = [
    { id: 'general', name: '일반', desc: 'GENERAL STYLE' },
        ...
        <main className="flex-1 flex overflow-hidden min-h-0">
            ...
            <div className="flex-1 flex items-center justify-center p-12">
              ...
              >
                ...
                <div className="mb-12">
                  ...
                  >
                    ...
                    <label htmlFor="video-upload" className="cursor-pointer">
                      <FileVideo
                        size={64}
                        className="text-slate-300 group-hover:text-brand-purple mx-auto mb-6"
                      />
                ...
                <div className="mb-14 flex flex-col gap-6">
                  <h3 className="text-xl font-bold">스타일 / 도메인 선택</h3>
          ...
          {phase === 'processing' && (
            <div className="flex-1 flex items-center justify-center">
              <Status progress={progress} jobStatus={jobStatus} />
            </div>
          )}
          ...
          {phase === 'done' && (
            <>
              <section className="flex-1 flex flex-col p-8 gap-8 min-w-0 h-full">
                ...
                <div
                  className={`h-32 rounded-3xl p-6 border shadow-inner overflow-hidden transition-all duration-500 ${theme.timeline}`}
                >
